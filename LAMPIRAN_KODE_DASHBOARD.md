# LAMPIRAN: Kode Dashboard Interaktif
**File:** `dashboard.py`  
...
1. [Import & Konfigurasi Page](#section-1)
2. [CSS Styling & Tema Premium](#section-2)
3. [Fungsi Loading Data](#section-3)
4. [Sidebar: Filter & Kontrol](#section-4)
5. [Header Utama](#section-5)
6. [Tab 1: Filter & Parameter (UC1)](#section-6)
7. [Tab 2: Peta Klaster Spasial (UC2)](#section-7)
8. [Tab 3: Heatmap Peluang (UC3)](#section-8)
9. [Tab 4: Statistik Efisiensi (UC4)](#section-9)
10. [Tab 5: Laporan Eksekutif (UC5)](#section-10)
11. [Footer](#section-11)

---

<a name="section-1"></a>
## 1️⃣ IMPORT & KONFIGURASI PAGE
...
<a name="section-2"></a>
## 2️⃣ CSS STYLING & TEMA PREMIUM

**Fungsi:** Styling UI dengan CSS custom (dark theme + responsive mobile)
...
**Penjelasan:**
- Dark theme dengan warna corporate blue (#00A6FB)
- Styling khusus untuk metric cards, tabs, dan charts
- Media query untuk responsive mobile (≤768px)
- Custom font: Plus Jakarta Sans

---

<a name="section-3"></a>
## 3️⃣ FUNGSI LOADING DATA
...
def load_geojson():
    ...
    if os.path.exists(file_path):
        with open(file_path, 'r') as f:
            ...
            # WORKAROUND & BUGFIX: 
            # 1. Hapus feature dengan geometry: null (Menyebabkan MapBox/Maplibre CRASH di JS)
            valid_features = []
...
<a name="section-4"></a>
## 4️⃣ SIDEBAR: FILTER & KONTROL

**Fungsi:** Sidebar dengan filter provinsi, kabupaten/kota, dan tombol Apply Filters
...
- Tombol "Apply Filters" untuk konfirmasi (pattern eksplisit)
- Info konteks wilayah terpilih
...
**Fungsi:** Judul halaman dan tab navigation
...
**Penjelasan:**
- Dynamic title yang menampilkan nama kota terpilih
- 5 tabs untuk 5 use case berbeda

---

<a name="section-6"></a>
## 6️⃣ TAB 1: FILTER & PARAMETER (UC1)

**Fungsi:** Menampilkan parameter global dan tabel data mentah
...
- Tabel raw data yang difilter sesuai wilayah terpilih
- Dynamic counter jumlah lowongan

---

<a name="section-7"></a>
## 7️⃣ TAB 2: PETA KLASTER SPASIAL (UC2)
...
with tab2:
    ...
    with comp_col3:
        st.metric("Cluster -1 — Isolated Zone", f"{len(cn)} wilayah", f"{int(cn['job_volume'].sum()):,} lowongan")
    
    st.info("""
    **💡 Catatan Analisis DBSCAN:** 
    * **Cluster 0 (Biru):** Aglomerasi pasar kerja koridor mainland Pulau Jawa yang terhubung secara kontigu — mencakup pusat regional seperti Surabaya, Bandung, Semarang,
 GKS, dan Solo.
...
- Legend horizontal di bawah peta
- 3 metric cards untuk ringkasan komposisi cluster
- Info box dengan interpretasi hasil DBSCAN

---

<a name="section-8"></a>
## 8️⃣ TAB 3: HEATMAP PELUANG (UC3)

**Fungsi:** Choropleth map untuk visualisasi Opportunity Index per wilayah
...
- Fallback ke density_map jika GeoJSON tidak ada

---

<a name="section-9"></a>
## 9️⃣ TAB 4: STATISTIK EFISIENSI (UC4)

**Fungsi:** Visualisasi korelasi dan metrik evaluasi model
...
with tab4:
    ...
    * Barchart Indeks Kompetitif pada level *>=2.5* didominasi posisi manajerial elit. Skor *~1.0 - 1.5* mengindikasikan pasar kerja kerah biru / peranan operasional.
    ...
    with m_col2:
        ...
        st.caption("ℹ️ *Nilai DBI 0.5294 (di bawah 1.0) menunjukkan pemisahan antar klaster yang baik — Cluster 0 (Mainland) dan Cluster 1 (Jabodetabek) tidak saling tumpang tindih secara spasial.*")
```

**Penjelasan:**
- 2 kolom: Scatter plot (efisiensi) + Bar chart (kompetitif)
- Trendline OLS untuk korelasi labor force vs job volume
- Metrik evaluasi DBSCAN: Silhouette Score & Davies-Bouldin Index

---

<a name="section-10"></a>
## 🔟 TAB 5: LAPORAN EKSEKUTIF (UC5)
...
with tab5:
    ...
    # Kalkulasi peringkat aktif (Eksklusi Wilayah dengan 0 Lowongan)
...
**Total Kode:** 553 baris  
**Total Section:** 11 bagian  
**Framework:** Streamlit 1.x + Plotly 5.x  
**Use Cases:** 5 (Filter, Map, Heatmap, Stats, Report)

---

## 📋 CARA MENGGUNAKAN KODE INI DI SKRIPSI

### **Opsi 1: Lampiran Lengkap (Semua Section)**
Masukkan semua 11 section di atas sebagai **Lampiran A**

### **Opsi 2: Lampiran Terpisah per Use Case**
- Lampiran A.1: Import & Config (Section 1-3)
- Lampiran A.2: Use Case 1 - Filter (Section 6)
- Lampiran A.3: Use Case 2 - Peta Klaster (Section 7)
- Lampiran A.4: Use Case 3 - Heatmap (Section 8)
- Lampiran A.5: Use Case 4 - Statistik (Section 9)
- Lampiran A.6: Use Case 5 - Laporan (Section 10)

### **Opsi 3: Lampiran Highlight Only**
Pilih 2-3 use case terpenting saja (misalnya UC2, UC3, UC5)

---

**Rekomendasi:** Gunakan **Opsi 1** untuk kelengkapan, atau **Opsi 2** jika dosen minta per-use-case terpisah.
